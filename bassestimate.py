...
class BassForecast:
    ...
    def one_step_ahead(self):
        ...
        mape = np.mean(
            np.abs(pred_cont - self.s[self.b_idx + 1 : self.e_idx + 1]) / self.s[self.b_idx + 1 : self.e_idx + 1]
        )
        mse = np.mean(np.sqrt(np.sum(np.square(pred_cont - self.s[self.b_idx + 1 : self.e_idx + 1]))))
        return [mad, mape, mse]

    def n_step_ahead(self):
        pred_cont = np.array([x[: self.n] for x in self.pred_res if self.n <= len(x)])
        act_cont = np.array([self.s[self.b_idx + i + 1 : self.b_idx + i + 1 + self.n] for i in range(len(pred_cont))])
...
if __name__ == "__main__":
    ...
    china_set = {
        "color tv": (
            np.arange(1997, 2013),
            [2.6, 1.2, 2.11, 3.79, 3.6, 7.33, 7.18, 5.29, 8.42, 5.68, 6.57, 5.49, 6.48, 5.42, 10.72, 5.15],
        ),
        "mobile phone": (
            np.arange(1997, 2013),
